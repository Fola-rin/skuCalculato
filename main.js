...
document.addEventListener("click", function (e) {
	...
	if (
		e.target.classList[0] === "show" ||
		e.target.classList[0] === "custom-arrow"
	) {
		e.target.parentNode.parentNode.classList.toggle("open");
		e.target.parentNode.classList.toggle("borderRadius");
	}
	if (e.target.classList[0] === "custom-option") {
		let customOptionSingle = e.target;
		customOptionSingle.parentNode.parentNode.classList.toggle("open");
		if (!customOptionSingle.classList.parentNode) {
			customOptionSingle.parentNode
				.querySelector(".custom-option.selected")
				.classList.remove("selected");
			customOptionSingle.classList.add("selected");
			customOptionSingle
				.closest(".custom-select")
				.querySelector(".custom-select__trigger .show").textContent =
				customOptionSingle.textContent;
		}
	}
	if (e.target.classList[0] === "stockCalc") {
		...
		let stockTakingAgents = parseInt(
			parentDiv.children[4].children[1].children[0].children[1].querySelector(
		...
		if (cost_title.children.length == 0) {
			cost_title.insertAdjacentHTML(
				"beforeend",
				`<span class = "costSpanMargin" id="costOF${
					parentDiv.children[0].id
				}" data-value="${singleCost}" data-timeLow ="${Math.floor(
					noOfDays
				)}" data-timeHigh="${Math.ceil(noOfDays)}"> ${
					parentDiv.children[0].value
				}: ${parentResultDiv.children[1].textContent}; </span>`
			);
			...
			for (const store of cost_title.children) {
				...
				if (store.id === `costOF${parentDiv.children[0].id}`) {
					cost_title.querySelector(
						`#costOF${parentDiv.children[0].id}`
					).textContent = ` ${parentDiv.children[0].value}: ${parentResultDiv.children[1].textContent};`;
					cost_title.querySelector(
						`#costOF${parentDiv.children[0].id}`
					).dataset.value = singleCost;
					cost_title.querySelector(
						`#costOF${parentDiv.children[0].id}`
					).dataset.timelow = Math.floor(noOfDays);
					cost_title.querySelector(
						`#costOF${parentDiv.children[0].id}`
					).dataset.timehigh = Math.ceil(noOfDays);
					console.log("red");
				} else if (
					store.id !== `costOF${parentDiv.children[0].id}` &&
					!Array.from(cost_title.children).includes(
						document.querySelector(`#costOF${parentDiv.children[0].id}`)
					)
				) {
					cost_title.insertAdjacentHTML(
						"beforeend",
	...
	if (e.target.classList[0] === "surveyCalc") {
		...
		if (cost_title.children.length == 0) {
			cost_title.insertAdjacentHTML(
				"beforeend",
				`<span class = "costSpanMargin" id="costOF${parentDiv.children[0].id}" data-value="${singleCost}"> ${parentDiv.children[0].value}: ${parentResultDiv.children[1].textContent};</span>`
			);
		} else {
			for (const store of cost_title.children) {
				// console.dir( cost_title.children[0].dataset)
...
document.addEventListener("mouseout", (e) => {
	if (e.target.classList[0] === "custom-option") {
		...
		// e.target.parentNode.firstElementChild.classList.toggle("borderRadius");
	}
});
